...
int main() {
    ...
    std::vector<std::string> queries = load_queries("queries.json", 30);
    ...
    // Warm-up LLM with 5 queries
    std::cout << "Warming up LLM with 5 queries..." << std::endl;
    for (int w = 0; w < 5; ++w) {
        generate_response(llm_ctx, llm_model, prompts[w], 50);
    }
    ...
    std::cout << "Running LLM generation for queries 5-24:\n";
