...
void FarmvilleApp::update(float timestep)
{
    ...
    for (const auto &[key, value] : map)
    {
        if (_elements.count(key) > 0)
        {
            ...
            // Apply flip to trucks (ID 6 and 7) and chickens (ID 0-3)
            if ((value.id >= 0 && value.id <= 3) || value.id == 6 || value.id == 7) {
                // Get ORIGINAL texture dimensions, not scaled element dimensions
                float texWidth = _elements[key]->getTexture()->getWidth();
                float texHeight = _elements[key]->getTexture()->getHeight();
                float baseScaleX = (float)value.width / texWidth;
                float baseScaleY = (float)value.height / texHeight;
                _elements[key]->setScale(baseScaleX * value.direction, baseScaleY);
            }
